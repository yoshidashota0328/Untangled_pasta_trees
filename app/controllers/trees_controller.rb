...
class TreesController < ApplicationController
  ...
  def create
    ...
    if @tree.save
      layer = current_user.layers.new(layer_id: 1, positionX: 500, positionY: 30, parent_id: 1,
                                      user_id: current_user.id, tree_id: @tree.id)
      layer.save!
  ...
  def update
    if @tree.update(set_tree_params)
      redirect_to user_path, success: 'Update successful.' unless request.referer&.include?('layers')
    else
